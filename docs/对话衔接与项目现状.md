...
5. 内联 JSON 先由 `ConvertFrom-Json` 校验，再写成工具自己创建的 UTF-8 无 BOM 临时
   ...
   用户文件。
6. 脚本白名单检查 `func`，`-ValidateOnly` 时调用 CLI 的 `--doctor` 静态预检；普通请求
   则先 `--start-host`，再以 `--request-file` 发给 CLI。`shutdown_host` 是例外：它不会为
   了关闭而新启动 Host。
7. `finally` 只删除本次脚本创建的临时请求文件，不碰工程、DPA 或用户文件。

因此，想增加一个正式函数时，不能只在 Rust 中实现：还要把函数加入这里的
`$allowedFunctions`，并同步更新 `SKILL.md`、测试和 Release 模板。
...
`src/` 和 `scripts/`，然后在 `%USERPROFILE%\.codex\skills\lgk-vector` 创建一个指向
中央源码的 Windows Junction。目标已经存在且不是指向同一源码时会拒绝，绝不覆盖。
